...
export default function PostCard({ post, index = 0, layout = 'card' }) {
    ...
    return (
      ...
      >
        <div className="flex flex-col md:flex-row items-stretch">
          <div className="w-full md:w-1/3 flex-shrink-0 overflow-hidden bg-[var(--color-bg)] rounded-t-2xl md:rounded-l-2xl md:rounded-tr-none">
            {post.image ? (
              <img
                src={post.image}
                alt={post.title}
                className="w-full h-full object-cover aspect-[4/3] md:aspect-auto group-hover:scale-105 transition-transform duration-500"
              />
            ) : (
          ...
          <div className="p-5 w-full">
            ...
            <h3 className="text-lg md:text-xl font-semibold leading-snug text-[var(--color-text)] group-hover:text-gray-600 transition-colors mb-2">
              {post.title}
            </h3>
            {post.excerpt && (
            ...
            <div className="flex items-center gap-2 mt-auto">
              {post.author?.avatar ? (
                ...
                <div className="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center text-[10px] font-bold text-gray-600">
                  {post.author?.name?.[0] || 'A'}
                </div>
              )}
              <span className="text-sm text-[var(--color-text-muted)]">{post.author?.name}</span>
              {post.readTime && (
                <>
