...
const seedData = async () => {
  ...
  for (const postData of posts) {
    // If Cloudinary is configured, upload remote image to Cloudinary (it will fetch the URL)
    if (
      ...
      try {
        const res = await cloudinary.uploader.upload(postData.image, {
          folder: 'blog-platform',
          transformation: [{ width: 1200, quality: 'auto', fetch_format: 'auto' }],
        });
        postData.image = res.secure_url;
        postData.imagePublicId = res.public_id;
      } catch (err) {
        console.warn('⚠️ Cloudinary upload failed, using original URL:', err.message);
      }
    }
